    ...
    private void OnValidate()
    {
        ...
        if (enemiesByLevelList.Count > 0 || roomEnemySpawnParametersList.Count > 0)
        {
            ...
            foreach (RoomEnemySpawnParameters roomEnemySpawnParameters in roomEnemySpawnParametersList)
            {
                ...
                foreach (SpawnableObjectsByLevel<EnemyDetails> dungeonObjectsByLevel in enemiesByLevelList)
                {
                    ...
                    foreach (SpawnableObjectRatio<EnemyDetails> dungeonObjectRatio in dungeonObjectsByLevel.spawnableObjectRatioList)
                    {
