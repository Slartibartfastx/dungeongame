...
public class DungeonBuilder : SingletonMonoBehaviour<DungeonBuilder>
{
    ...
    private NodeTypeListScriptableObject roomNodeTypeList;
    ...
    /// </summary>
    public bool GenerateDungeon(DungeonLevelScriptableObject currentDungeonLevel)
    {
        ...
        while (!dungeonBuildSuccessful && dungeonBuildAttempts < Settings.maxDungeonBuildAttempts)
        {
            ...
            NodeGraphObject roomNodeGraph = SelectRandomRoomNodeGraph(currentDungeonLevel.roomNodeGraphList);
    ...
    /// </summary>
    private NodeGraphObject SelectRandomRoomNodeGraph(List<NodeGraphObject> roomNodeGraphList)
    {
    ...
    private bool AttemptToBuildRandomDungeon(NodeGraphObject roomNodeGraph)
    {
        // Create Open Room Node Queue
        Queue<NodeScriptableObject> openRoomNodeQueue = new Queue<NodeScriptableObject>();

        // Ensure roomNodeTypeList is not null or empty
        if (roomNodeTypeList.list == null || roomNodeTypeList.list.Count == 0)
        {
            Debug.LogError("roomNodeTypeList is empty or null.");
            return false; // Handle appropriately
        }


        // Find the Entrance node type in the list
        NodeScriptableObject entranceNode = roomNodeGraph.GetRoomNode(
    roomNodeTypeList.list.Find(x => x.roomType == RoomType.Entrance).roomType
);
        ...
        if (entranceNode != null)
            ...
            {
                Debug.LogError("Entrance node was not found in the room node graph.");
    ...
    /// </summary>
    private bool ProcessRoomsInOpenRoomNodeQueue(NodeGraphObject roomNodeGraph, Queue<NodeScriptableObject> openRoomNodeQueue, bool noRoomOverlaps)
    {
        ...
        while (openRoomNodeQueue.Count > 0 && noRoomOverlaps == true)
        {
            ...
            NodeScriptableObject roomNode = openRoomNodeQueue.Dequeue();

            // Add child Nodes to queue from room node graph (with links to this parent Room)
            foreach (NodeScriptableObject childRoomNode in roomNodeGraph.GetChildRoomNodes(roomNode))
            {
                openRoomNodeQueue.Enqueue(childRoomNode);
            }

            // if the room is the entrance mark as positioned and add to room dictionary
            if (roomNode.roomNodeType.roomType == RoomType.Entrance)
            {
    ...
    /// </summary>
    private bool CanPlaceRoomWithNoOverlaps(NodeScriptableObject roomNode, Room parentRoom)
    {
    ...
    /// </summary>
    private RoomTemplateSO GetRandomTemplateForRoomConsistentWithParent(NodeScriptableObject roomNode, Doorway doorwayParent)
    {
        RoomTemplateSO roomtemplate = null;

        // If room node is a corridor then select random correct Corridor room template based on
        // parent doorway orientation
        if (roomNode.roomNodeType.roomType == RoomType.Corridor)
        {
            switch (doorwayParent.orientation)
            {
                case Orientation.north:
                case Orientation.south:
                    roomtemplate = GetRandomRoomTemplate(roomNodeTypeList.list.Find(x => x.roomType == RoomType.CorridorNS));
                    break;


                case Orientation.east:
                case Orientation.west:
                    roomtemplate = GetRandomRoomTemplate(roomNodeTypeList.list.Find(x => x.roomType == RoomType.CorridorEW));
                    break;
    ...
    /// </summary>
    private RoomTemplateSO GetRandomRoomTemplate(NodeTypeScriptableObject roomNodeType)
    {
    ...
    /// </summary>
    private Room CreateRoomFromRoomTemplate(RoomTemplateSO roomTemplate, NodeScriptableObject roomNode)
    {
        ...
        if (roomNode.parentRoomNodeIDList.Count == 0) // Entrance
        {
            ...
            GameManager.Instance.setRoom(room);
        }
        else
        {
